...
Модуль PowerShell для загрузки данных из файлов CSV и Excel в датасет Apache Superset через REST API. Командлет `Send-ToApacheDataset` выполняет аутентификацию, проверяет наличие указанной базы данных и датасета, затем загружает данные с возможностью замены существующих записей.
